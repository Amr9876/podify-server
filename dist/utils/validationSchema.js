...
exports.CreateUserSchema = yup.object().shape({
    ...
    password: yup
        ...
        .matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+-={}|;:,.<>?]).{8,32}$/, "Password should contain atleast one capital letter, numeric character and a special character"),
});
...
exports.NewPlaylistValidationSchema = yup.object().shape({
    ...
    visibility: yup
        .string()
        .oneOf(["public", "private"], "Invalid Option!")
        .required("Visibility option is missing!"),
});
exports.OldPlaylistValidationSchema = yup.object().shape({
    ...
    visibility: yup.string().oneOf(["public", "private"], "Invalid Option!"),
});
